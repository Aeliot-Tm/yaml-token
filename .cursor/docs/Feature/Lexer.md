...
- **Directives** (YAML 1.2.2 §6.8, §9.1.2, §9.1.3):
  ...
  - `%TAG` lines: when the keyword is followed by horizontal whitespace, `!` (start of a tag handle),
    ...
    `TAG_HANDLE` (`!`, `!!`, or `!name!`), optional `WHITESPACE`, `TAG_PREFIX`
...
- **Anchors, aliases, tags**:
  - `&name` → `ANCHOR_PROPERTY`, `*name` → `ALIAS_NODE`
  - Explicit tag property at a node (not `%TAG` directive lines): one `TAG_PROPERTY` token with the full lexeme
