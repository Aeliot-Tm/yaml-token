...
Parser error messages append a human-readable source position (`in line … column …`)
so failures point at the right place in the input.

## Behaviour

When the parser throws, the message is passed through
[`ErrorHelper::appendTokenLocation()`](../../../src/Parser/Helper/ErrorHelper.php).
...
1. **A `Token` instance** — when the call site already holds a non-null reference
   to the token that explains the error (the lexer’s `line` and `column` on that
...
might be `null`, pass the proxy directly; otherwise pass the concrete `Token`.
